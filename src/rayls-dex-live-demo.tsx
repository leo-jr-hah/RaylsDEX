...
import { ArrowDownUp, Shield, Building2, Users, Lock, Eye, Zap, TrendingUp } from 'lucide-react';
import './index.css';
...
export default function RaylsDEX() {
  ...
  const publicTokens = [
    { symbol: 'BTC', name: 'Bitcoin', type: 'Crypto', liquidity: '$85.6M', description: 'The original cryptocurrency - digital gold for the modern era' },
    { symbol: 'ETH', name: 'Ethereum', type: 'Crypto', liquidity: '$72.3M', description: 'Smart contract platform powering DeFi applications' },
    { symbol: 'USDC', name: 'USD Coin', type: 'Stablecoin', liquidity: '$45.2M', description: '1:1 backed by US dollars - stable value for trading' },
    { symbol: 'RAYLS', name: 'Rayls Token', type: 'Native', liquidity: '$23M', description: 'Native token of the Rayls ecosystem' },
    { symbol: 'SOL', name: 'Solana', type: 'Crypto', liquidity: '$36.5M', description: 'High-performance blockchain for fast transactions' }
  ];

  const privateTokens = [
    { symbol: 'BANK_POOL', name: 'Institutional Deposit Pool', type: 'Institutional', liquidity: 'Hidden', description: 'Bank deposits tokenized for institutional trading - private and compliant' },
    { symbol: 'GOV_TBILL', name: 'Tokenized Treasury Bills', type: 'RWA', liquidity: 'Hidden', description: 'US government treasury bills on blockchain - institutional grade yield' },
    { symbol: 'CORP_BOND', name: 'Corporate Bond Pool', type: 'RWA', liquidity: 'Hidden', description: 'Corporate bonds from major companies - private settlement' },
    { symbol: 'CBDC_USD', name: 'CBDC Settlement Pool', type: 'CBDC', liquidity: 'Hidden', description: 'Central Bank Digital Currency for instant cross-border settlement' }
  ];
      ...
      <div className="max-w-6xl mx-auto mb-8">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-gray-300">
                <strong>Hybrid Architecture:</strong> Trade on the Public Chain (transparent, KYC-verified) or Private Network (confidential, institutional-grade). Powered by Enygma privacy protocol.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            {activeTab !== 'public' && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-slate-800 border border-slate-700 rounded-lg p-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
                <p className="text-xs text-gray-300">
                  <strong>Public Chain:</strong> Like any crypto exchange - your trades are visible, all users are verified by KYC. Perfect for regular crypto trading and DeFi apps.
                </p>
              </div>
            )}
          </button>
          ...
          >
            ...
            <p className="text-xs opacity-80">Banks & institutions only - fully private</p>
            {activeTab !== 'private' && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-slate-800 border border-slate-700 rounded-lg p-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
                <p className="text-xs text-gray-300">
                  <strong>Private Chain:</strong> For banks and big institutions - trades are completely confidential. Think of it like having a private banking network on blockchain.
                </p>
              </div>
            )}
          </button>
          ...
          <div className="md:col-span-2 bg-slate-800/50 backdrop-blur rounded-2xl border border-slate-700 p-6">
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              ...
              {activeTab === 'private' && (
                <button
                  ...
                  title={showPrivacyMode ? "Your trade details are encrypted - only you and regulators can see" : "Click to enable privacy mode"}
                >
            ...
            <div className="bg-slate-900/50 rounded-xl p-4 mb-6">
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-gray-400">
                  {activeTab === 'private' && showPrivacyMode ? 'Balance: Hidden' : 'Balance: 5,420'}
                </span>
              </div>
              <div className="flex gap-3 flex-wrap">
                <input
            ...
            {amount && (
              <div className="bg-slate-900/30 rounded-lg p-4 mb-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-right">1 {selectedFromToken} = 0.98 {selectedToToken}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Fee</span>
                  <span>{activeTab === 'private' ? '0.05% (Institutional)' : '0.3%'}</span>
                </div>
                <div className="flex justify-between">
            ...
            {activeTab === 'private' && (
              <p className="text-xs text-center text-gray-400 mt-3">
                Transaction will be settled via Private Network Hub with ZK-proof verification
              </p>
            )}
          </div>
                ...
                >
                  <div className="flex justify-between items-start mb-2 gap-2">
                    ...
                    <div className={`text-xs px-2 py-0.5 rounded flex-shrink-0 ${
                      token.type === 'RWA' ? 'bg-orange-500/20 text-orange-400' :
                      token.type === 'Institutional' ? 'bg-purple-500/20 text-purple-400' :
                      token.type === 'CBDC' ? 'bg-green-500/20 text-green-400' :
                      token.type === 'Crypto' ? 'bg-cyan-500/20 text-cyan-400' :
            ...
            {activeTab === 'private' && (
              <div className="mt-4 bg-purple-500/10 border border-purple-500/30 rounded-lg p-3">
                <p className="text-xs text-gray-300">
                  ...
                  Institutional pools are privacy-protected. Only authorized participants can view full order books.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6">
          <div className="group relative bg-gradient-to-br from-blue-500/10 to-cyan-500/10 border border-blue-500/30 rounded-xl p-4 hover:border-blue-500/50 transition-colors cursor-pointer">
            ...
            <p className="text-sm text-gray-400">ZK-proof shielded orders with auditable compliance</p>
            <div className="absolute left-0 right-0 top-full mt-2 bg-slate-800 border border-blue-500/30 rounded-lg p-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
              <p className="text-xs text-gray-300">
                <strong>What's this?</strong> Advanced encryption that keeps your trades private while letting regulators verify everything is legal. Best of both worlds!
              </p>
            </div>
          </div>
          <div className="group relative bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-xl p-4 hover:border-purple-500/50 transition-colors cursor-pointer">
            ...
            <p className="text-sm text-gray-400">Tokenized deposits, treasuries, and real estate</p>
            <div className="absolute left-0 right-0 top-full mt-2 bg-slate-800 border border-purple-500/30 rounded-lg p-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
              <p className="text-xs text-gray-300">
                <strong>RWA = Real World Assets:</strong> Trade real things like bank deposits, government bonds, and real estate on blockchain. It's like bringing Wall Street to crypto!
              </p>
            </div>
          </div>
          <div className="group relative bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-xl p-4 hover:border-green-500/50 transition-colors cursor-pointer sm:col-span-2 md:col-span-1">
            ...
            <p className="text-sm text-gray-400">Institutional-grade settlement speed</p>
            <div className="absolute left-0 right-0 top-full mt-2 bg-slate-800 border border-green-500/30 rounded-lg p-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
              <p className="text-xs text-gray-300">
                <strong>TPS = Transactions Per Second:</strong> Can handle 10,000+ trades every second. That's faster than Visa! Your trades settle in about 3 seconds.
              </p>
            </div>
          </div>
        </div>
